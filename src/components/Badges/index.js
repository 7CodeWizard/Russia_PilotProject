...
const CaseCatalogCardBadge = ({ type, label }) => {
  ...
  return (
    <>
      {type === "platform" ?
        <button className="caseCatalogCardBadgeButton" style={{ display: "flex", gap: "10px" }}        >
          <img src={whiteUser} alt="whiteUser" /> {label}
        </button>
        : type === "equipment" ?
          <button className="caseCatalogCardBadgeButton" style={{ backgroundColor: '#EFEFEF', color: '#686868' }}        >
            {label}
          </button>
          :
...
export { CardViewNumber, CardBadge, CaseCatalogCardBadge, UserCardNumber }