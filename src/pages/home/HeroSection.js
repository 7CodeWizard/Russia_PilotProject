      ...
      <div className="heroSection ">
        <div className="section1 heroContent">
            ...
            {menuItemsData[1].submenu.map((item, index) => (
              <div key={index} className={`alignCenter ${buttonGroupClassInfo[index]}`} style={{ gap: '5px' }}>
                ...
                {item.submenu.map((menu, idx) => (
                  <SmallTabButton onClick={() => handleLink(menu.url)} key={idx} title={menu.title} />
                ))}
              </div>
            ))}
          </div>
          ...
          <div className="flexWrap itemCenter heroBottomButton">
            <Link to="gallerySection" spy={true} smooth={true}><DefaultButton title='лУЧШИЕ КЕЙСЫ' /></Link>
            <BlackButton title='зАПОЛНИТЬ БРИФ' onClick={() => handleNavigation('https://forms.yandex.com/admin/')} />
          </div>
        </div>
      </div>
      <div>
